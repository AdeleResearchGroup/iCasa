...
public class DeviceDependencyHandler extends DependencyHandler implements ApplicationListener {
    ...
    /**
     * The application id associated to the component associated to this handler
     */
        ...
        for (int i = 0; callbacks != null && i < callbacks.length; i++) {
            ...
            if (mets.length == 0) {
                ...
                if (mets[0].getMethodArguments().length == 1) {
                    ...
                    // The callback receives service object, service reference. Check that the second argument is a
                    // service
                    // reference
                    if (!(mets[0].getMethodArguments()[1].equals(ServiceReference.class.getName()) // callback with
                                                                                                   // (service
                                                                                                   // object, service
                                                                                                   // reference)
                            || mets[0].getMethodArguments()[1].equals(Dictionary.class.getName()) // callback with
                                                                                                  // (service
                                                                                                  // object, service
                                                                                                  // properties
                                                                                                  // in a dictionary)
            ...
            if (type.endsWith("[]")) {
                ...
                if (dep.isAggregate()) {
                    throw new ConfigurationException("A required service is not correct : the field "
                            + meta.getFieldName() + " must be an array to support aggregate injections");
                }
            }
                ...
                if (type.endsWith("[]")) {
                    ...
                    if (dep.isAggregate()) {
                        throw new ConfigurationException(
                                "A required service is not correct : the constructor parameter " + index
                ...
                if (dep.getSpecification() != null) {
                    if (error) {
                        ...
                        warn("[" + getInstanceManager().getInstanceName() + "] The field type [" + className
                                + "] and the required service interface [" + dep.getSpecification()
                                + "] are not the same");
                    }
                }
    ...
    public void configure(Element componentMetadata, Dictionary configuration) throws ConfigurationException {
        ...
        Map<String, DependencyConfiguration> tempDependencies = new HashMap<String, DependencyConfiguration>();
        ...
        for (int i = 0; deps != null && i < deps.length; i++) {
            ...
            if (type.equals("bind") || type.equals("unbind")) {
                ...
                throw new ConfigurationException(
                        "A requires-device dependency must contain a type field, bind or unbind");
            }
            ...
            DependencyConfiguration existingDependency = tempDependencies.get(identity);
            ...
            if (existingDependency != null) {
                if (existingDependency.type.equals("field")) {
                    ...
                    if (type.equals("field")) {
                        existingDependency.completeDependency(field, type, aggregate, specification, filter, context,
                                comparator, policy, optional, mandatoryProps);
                ...
                DependencyConfiguration temporalDependency = new DependencyConfiguration(identity);
        ...
        for (DependencyConfiguration tempDependency : tempDependencies.values()) {

            DeviceDependency dep = new DeviceDependency(this, tempDependency.field, tempDependency.specification,
                    tempDependency.filter, tempDependency.optional, tempDependency.aggregate, tempDependency.id,
                    tempDependency.context, tempDependency.policy, tempDependency.comparator,
                    tempDependency.mandatoryProps);

            for (String method : tempDependency.bindCallbacks) {
                dep.addDependencyCallback(new DependencyCallback(dep, method, DeviceDependency.BIND_ICASA));
            }
            for (String method : tempDependency.unbindCallbacks) {
    ...
    public void start() {
        ...
        m_applicationManager.addApplicationListener(this);
    }
    ...
    public void stop() {
        ...
        m_applicationManager.removeApplicationListener(this);
    }
    ...
    /*
     * public AccessRight getAccessRight(BundleContext context, String deviceId) { String appId =
     * getApplicationId(context); if (appId != null) { return accessManager.getAccessRight(appId, deviceId); } return
     * null; }
     */
    ...
    public String getApplicationId(BundleContext context) {
        if (m_applicationId == null) {
            ...
            Application app = m_applicationManager.getApplicationOfBundle(bundleName);
            if (app != null) {
    ...
    private void setApplicationId(String newId) {
        // Only if The application Id has not been set
        if (m_applicationId==null) {
            m_applicationId = newId;
            // The handler is no more interested in events from the application manager
            m_applicationManager.removeApplicationListener(this);            
        }
    }
    ...
    public void bundleAdded(Application app, String symbolicName) {
        ...
        String ownSymbolicName = getInstanceManager().getContext().getBundle().getSymbolicName();
        if (ownSymbolicName.equals(symbolicName)) {
            setApplicationId(app.getId());
            for (DeviceDependency dep : m_dependencies) {
    ...
    private class DependencyConfiguration {
        ...
        private String mandatoryProps;

        List<String> bindCallbacks = new ArrayList<String>();
        List<String> unbindCallbacks = new ArrayList<String>();

        public DependencyConfiguration(String id) {
            this.id = id;
        }

        public void completeDependency(String field, String type, boolean aggregate, Class specification,
                Filter filter, BundleContext context, Comparator comparator, int policy, boolean optional,
